    ...
    <section ref={containerRef} className="relative h-screen min-h-[700px] overflow-hidden bg-white">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <AnimatePresence mode="sync">
          <motion.img key={currentSlide} src={heroImages[currentSlide]} alt="Living Rock Events - Wedding & Events Venue"
      ...
      <motion.div className="relative z-20 flex flex-col justify-center h-full max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 pt-36"
        ...
        <div className="overflow-hidden">
          {(hero.titleLines || ['YOUR DREAM', '', 'EVENT.']).map((line, i) => (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }}
          ...
          <p className="text-neutral-900/30 text-xs sm:text-sm uppercase tracking-[0.2em]" style={{ fontFamily: 'var(--font-sans)' }}>
            {hero.trustLine || '4.3 Stars \u00b7 72+ Reviews \u00b7 Harare'}
          </p>
        </motion.div>
...
function StatsBand() {
  ...
  const ds = stats?.length ? stats : [{ number: '72', label: 'Reviews' }, { number: '200', label: 'Events' }, { number: '4.3', label: 'Rating' }, { number: '5', label: 'Years' }];
  return (
...
function CTASection() {
  ...
  return (
    <section ref={ref} className="relative py-28 sm:py-36 lg:py-48 overflow-hidden">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <img src={homeCta?.backgroundImage || 'https://images.unsplash.com/photo-1505236858219-8359eb29e329?w=1200&q=80'} alt="Living Rock Events - Wedding & Events Venue" className="w-full h-[130%] object-cover object-center" loading="eager" />
      ...
      <div className="relative z-20 max-w-5xl mx-auto px-5 sm:px-8 lg:px-12 text-center">
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 1 }}>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <a href={`https://wa.me/${business.whatsappNumber || '263775601556'}`} target="_blank" rel="noopener noreferrer"
